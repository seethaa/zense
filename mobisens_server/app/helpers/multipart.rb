...
module Multipart
  ...
  def self.post_form(url, query, headers)
    ...
    res = Net::HTTP.start(url.host, url.port) {|con|
      con.read_timeout = 10 * 60 * 60
      begin
        return_obj = con.post(url.path, query, headers)
      rescue => e
        content = $!
        exception = e
        return_obj = -1
    ...
    return {:status => return_obj, :content => content, :exception => exception}
  end
